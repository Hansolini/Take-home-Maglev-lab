...
% temp = zeros(3,params.magnet.n)
% I = params.permanent.J/params.physical.mu0*params.permanent.l(1);
% 
% for i = 1:params.magnet.n
%     temp(1,i) = bxBase(i)*I+bxBase(i+params.magnet.n)*I+bxBase(i+2*params.magnet.n)*I + bxBase(i+3*params.magnet.n)*I;
%     temp(2,i) = byBase(i)*I+bxBase(i+params.magnet.n)*I+byBase(i+2*params.magnet.n)*I + byBase(i+3*params.magnet.n)*I;
%     temp(3,i) = bzBase(i)*I+bxBase(i+params.magnet.n)*I+bzBase(i+2*params.magnet.n)*I + bzBase(i+3*params.magnet.n)*I;
% end
% bxBase = temp(1,:);
% byBase = temp(2,:);
% bzBase = temp(3,:);
...
bMagnet = R*[bxMagnet; byMagnet; bzMagnet].*I;
bxMagnet = bMagnet(1);
byMagnet = bMagnet(2);
bzMagnet = bMagnet(3);
