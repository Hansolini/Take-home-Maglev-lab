function [bx_p,by_p,bz_p,bx_s,by_s,bz_s] = computeFieldBase(x,y,z,u,params,modelName)
...
bx_p = zeros(size(x,1),4*size(x,2));
by_p = zeros(size(x,1),4*size(y,2));
bz_p = zeros(size(x,1),4*size(z,2));

bx_s = zeros(size(x,1),4*size(x,2));
by_s = zeros(size(x,1),4*size(y,2));
bz_s = zeros(size(x,1),4*size(z,2));
...
switch modelName
    case {'filament','accurate'}
        for i = 1:length(params.permanent.r)
            ...
            bx_p(1+(i-1)*length(x):i*length(x)) = bxTemp;
            by_p(1+(i-1)*length(x):i*length(x)) = byTemp;
            bz_p(1+(i-1)*length(x):i*length(x)) = bzTemp;
        end
...
switch modelName
        ...
        for i = 1:length(params.solenoids.r)
            ...
            for j = 1:nwAxial
                for k = 2:nwRadial % Looping from 2 to 'skip' radius=0 (also added one extra in the radial direction to compensate)
                    ...
                    bx_s(1+(i-1)*length(x):i*length(x)) = bx_s(1+(i-1)*length(x):i*length(x)) + bxTemp;
                    by_s(1+(i-1)*length(x):i*length(x)) = by_s(1+(i-1)*length(x):i*length(x)) + byTemp;
                    bz_s(1+(i-1)*length(x):i*length(x)) = bz_s(1+(i-1)*length(x):i*length(x)) + bzTemp;
                end
            end
        end
    ...
    case 'accurate'
        ...
        for i = 1:length(params.solenoids.r)
            ...
            bx_s(1+(i-1)*length(x):i*length(x)) = bxTemp*params.solenoids.nw;
            by_s(1+(i-1)*length(x):i*length(x)) = byTemp*params.solenoids.nw;
            bz_s(1+(i-1)*length(x):i*length(x)) = bzTemp*params.solenoids.nw;
        end
