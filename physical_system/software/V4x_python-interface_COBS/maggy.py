...
'''
    API for scripts that want to import this module:
        ...
        set_solenoid_currents(currents : Dict[str, int] -> True: correctly set ; False: errors occured during setting
...
class MaglevInterface:
    ...
    def _send_command(self, cmd_name: str, parameters: Optional[Dict[str, Any]] = None) -> bytes:
        ...
        if cmd_name == "READ_SENSORS":
            data = bytes([0x01])
        elif cmd_name == "READ_CURRENTS":
            data = bytes([0x02])
        elif cmd_name == "SET_CURRENTS":
            data = bytes([0x03, parameters["x+"], parameters["y-"], parameters["x-"], parameters["y+"]])
        elif cmd_name == "RESET_CURRENTS":
            data = bytes([0x04])
    ...
    def get_position(self) -> Optional[Dict[str, float]]:
        try:
            ...
            if response is None:
                ...
                x, y, z = struct.unpack('@fff', response[1:])   # thanks to '@' this code adapt to the architecture's endianness
    ...
    def set_solenoid_currents(self, currents: Dict[str, int]) -> bool:
        ...
        try:
            response = self._send_command("SET_CURRENTS", currents)
